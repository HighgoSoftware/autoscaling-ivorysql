...
func makeAutoscaleEnforcerPlugin(
	...
	initEventsDone := make(chan struct{})
	incEventCountIfInit := func(isInit bool) {
		if isInit {
			initEventsCount.Add(1)
		}
	}
	decEventCountIfInit := func(isInit bool) {
		if isInit {
			newCount := initEventsCount.Add(-1)
			if newCount == 0 {
				close(initEventsDone)
			}
		}
	}
	...
	pwc := podWatchCallbacks{
		submitStarted: func(logger *zap.Logger, pod *corev1.Pod, preexisting bool) {
			incEventCountIfInit(preexisting)
			pushToQueue(logger, pod.Name, func() {
				p.handleStarted(hlogger, pod)
				decEventCountIfInit(preexisting)
			})
		},
